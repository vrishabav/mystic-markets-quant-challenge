...
Implements VWM(2,1) for Solaris and Mom(10) for Infinity Stones
"""
...
def strategy_Compound_V_Vials(bar, _s={"c": []}):
    ...
    diff = _s["c"][-1] - _s["c"][0] # Mom(3)
    return "LONG" if diff > 0 else ("SHORT" if diff < 0 else "FLAT")

def strategy_Solaris_Stardust(bar, _s={"c": [], "v": []}):
    _s["c"].append(float(bar["close"]))
    ...
    # VWM sum2, diff1
    val = (_s["c"][-1] - _s["c"][-2]) * _s["v"][-1] + (_s["c"][-2] - _s["c"][-3]) * _s["v"][-2]
...
def strategy_Infinity_Stones(bar, _s={"c": []}):
    _s["c"].append(float(bar["close"]))
    if len(_s["c"]) > 11: _s["c"].pop(0)
    if len(_s["c"]) < 11: return "FLAT"
    diff = _s["c"][-1] - _s["c"][0] # Mom(10)
    return "LONG" if diff > 0 else ("SHORT" if diff < 0 else "FLAT")
